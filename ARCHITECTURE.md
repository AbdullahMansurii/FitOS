...
The `authStore` uses `partialize` to exclude `isUnlocked` from persistence, forcing re-authentication on every session.
...
|------|----------------|------------|
| **Auth** | bcrypt-hashed master password in localStorage | ⚠️ Medium — localStorage is accessible via browser DevTools |
...
| **Supabase RLS** | Enabled on all tables. Authorization token `x-fitos-auth` validated against profile's `sync_token`. | 🟢 Low — remote database queries are secure. |
...
8. **Data export.** CSV/JSON export for weight logs, food logs, and workout sessions.
